...
//    printf("%llu\n", strlen(words));
    if (words[strlen(words)] != '\0') {
        printf("String does not end with a null character");
        exit(1);
    }
    ...
    // Creates the array with the returning strings, with the size equal to the words counted.
    str *subStrngs = (str *)malloc(wordCount);
    if (subStrngs == NULL) {
        printf("Memory allocation failed\n");
        exit(1);
    }

    // Adds all the strings to the sub array.
    for (int i = 0; i < wordCount; ++i) {
        strcpy(subStrngs[i].word, tempSubStrings[i].word);
        printf("%s\n", tempSubStrings[i].word);
    }
